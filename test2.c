...
int main(int argc, char **argv) {
	...
	if(argc > 5) {
		...
		*/
		...
		size_t lsz = GSZ;
		size_t in_data_sz = sizeof(T) * data_size / 2;
		size_t out_data_sz = sizeof(T) * data_size;
		T *in_data0, *in_data1, *out_data;
		cl_mem in_data0_mem, in_data1_mem, out_data_mem;
		...
		out_data_mem = clCreateBuffer(context, CL_MEM_BANK_1_ALTERA | CL_MEM_WRITE_ONLY, out_data_sz, NULL, &ret2);
		if(ret0 != CL_SUCCESS | ret1 != CL_SUCCESS | ret2 != CL_SUCCESS) {
			printf("ERROR in clCreateBuffer(): %s, %s, %s\n", oclReturnCodeToString(ret0), oclReturnCodeToString(ret1), oclReturnCodeToString(ret2));
			return -1;
		}
		...
		*/
		...
		ret2 = clSetKernelArg(kernel, 2, sizeof(cl_mem), (void *)&out_data_mem);
		if(ret0 != CL_SUCCESS | ret1 != CL_SUCCESS | ret2 != CL_SUCCESS) {
		...
		for(i = 0; i < iter; i++) {
			...
			*/
			ret = clEnqueueReadBuffer(queue, out_data_mem, CL_TRUE, 0, out_data_sz, out_data, 0, NULL, NULL);
			if(ret != CL_SUCCESS) {
			...
			errors[i] = 0;
			for(j = 0; j < GSZ ; j++) {
				errors[i] += ((in_data0[j] + in_data1[j]) != out_data[j]);
			}
