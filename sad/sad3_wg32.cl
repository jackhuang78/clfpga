...
void sad(__global T * image, __global T * filter, __global T * out) {
	...
	unsigned int global_r = get_global_id(1);//get_group_id(1) * LOCAL_S + get_local_id(1);
	unsigned int global_c = get_global_id(0);//get_group_id(0) * LOCAL_S + get_local_id(0);
...
/*
	...
	if(local_r < extra_s) {
		...
		image_temp[local_c[pLOCAL_S + local_r] = IMAGE(group_r + local_c, LOCAL_S + group_c + local_r);
